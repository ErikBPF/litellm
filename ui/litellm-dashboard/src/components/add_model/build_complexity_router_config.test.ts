...
describe("custom tier sets", () => {
  ...
  // A hand-written config keys `tiers` and `fallback_tier` in any case, which the backend resolves
  // with casefold, so hydration must too or the pools come back empty and the floor reads as off.
  ...
  it("getCustomTierRowsError reports the first per-row gap and passes a complete set", () => {
    ...
    expect(withRows([{ id: "a", name: " ", definition: "d", models: ["m"] }])).toBe("Name every tier");
    expect(withRows([{ id: "a", name: "A", definition: " ", models: ["m"] }])).toContain("definition");
    ...
    // Case-colliding names pass the backend's casefold uniqueness check only if we fold too.
