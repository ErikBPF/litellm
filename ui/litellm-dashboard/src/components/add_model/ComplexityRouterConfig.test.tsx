...
describe("edit tiers", () => {
  ...
  // Remove snapshots the row's in-editor models so Restore returns those, not the stale prop pool,
  // and only "Use built-in tiers" leaves the edited set, returning the pristine value.
  // One editing pass over the whole surface: rows expose name/definition with the backend's limits and
  // no newlines, rules follow a rename (they point by NAME, unlike the id-based fallback and floor),
  // and every input a custom set forces off is disabled with a reason instead of silently stripped.
  ...
  it("removes, restores, and exits the tier set without losing in-editor models", async () => {
    ...
    const removed = await step(defaultValue, "Remove the MEDIUM tier");
